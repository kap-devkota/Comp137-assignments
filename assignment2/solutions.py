...
class DenseLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, input_dim, output_dim, activation, reg_weight, param_init=None):
        ...
        if param_init is None or param_init == "autograder":
            param_init = dict()
            np.random.seed(137)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        self.add_loss(lambda : self.reg_weight * tf.reduce_sum(tf.square(self.W)))
...
class Feedforward(tf.keras.Model):
    ...
    def __init__(self, input_size, depth, hidden_sizes, output_size, reg_weight, task_type):
        ...
        for i in range(depth - 1):
            self.denselayers.append(DenseLayer(input_sizes[i], output_sizes[i], activation='relu', reg_weight=reg_weight))
